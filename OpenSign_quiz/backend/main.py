...
- MediaPipe Holistic은 정적 이미지 모드에서 프레임 단위로 처리한다.
"""
...
# 3) 데이터 모델 : 예측 요청 스키마 정의
class PredictRequest(BaseModel):
    category: str
    frames: List[str]  # base64 PNG 90개


# 4) 라벨 반환 API : 카테고리별 학습 라벨 목록 반환
@app.get("/labels")
...
# 6) 예측 API : 90프레임 시퀀스 → 특징 추출 → GRU(v2) 예측 → 라벨/확률 반환
@app.post("/predict")
